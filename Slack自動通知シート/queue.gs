...
function nightlyScanAndQueue() {
  ...
  try {
    ...
    ensureHeaders_(queueSheet, Object.values(QUEUE_HEADERS));
    ...
    for (const cfg of configs) {
      ...
      for (let r = 0; r < staffNames.length; r++) {
        ...
        for (let c = 0; c < taskNames.length; c++) {
          processedCells++;
          if (processedCells > APP_CONFIG.NIGHTLY_SCAN_CELL_LIMIT) { reachedLimit = true; break; }
    ...
    notifyUser_(`夜間スキャンに失敗しました: ${e.message}`, 'ERROR');
    return;
  }

  if (reachedLimit) {
    notifyUser_(`夜間スキャンを一時停止: キュー追加=${queued}（上限に達しました）`, 'WARN');
  } else {
    notifyUser_(`夜間スキャン完了: キュー追加=${queued}`, 'INFO');
  }
}

/** 朝の送信（QueueからPENDINGを送るだけ） */
function morningDispatch() {
  notifyUser_('朝の送信を開始します...', 'INFO');
  let sent = 0;
  try {
    const qsh = getActive_().getSheetByName(SHEET_NAMES.QUEUE);
    if (!qsh) {
      notifyUser_('Queueシートが見つかりませんでした', 'WARN');
      return;
    }
    ...
    notifyUser_(`朝の送信に失敗しました: ${e.message}`, 'ERROR');
    return;
  }
  notifyUser_(`朝の送信完了: 送信=${sent}`, 'INFO');
}
